...
ipcRenderer.on('builds-loaded', (event, builds) => {
    ...
    builds.forEach(build => {
        ...
        buildElement.innerHTML = `
            ...
            <div class="mod_info">
                ...
                <div class="Описание">${build.description}</div>
    ...
    // Показываем кнопку "Добавить новую сборку", если сборки уже существуют
    const addBuildButton = document.getElementById('add-build-button');
    if (builds.length > 0) {
        addBuildButton.style.display = 'block';
    } else {
        addBuildButton.style.display = 'none';
    }
});
